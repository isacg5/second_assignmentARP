...
// FUNCTION FOR SHARED MEMORY
void write_on_shared_mem(int *ptr, int radius, int x_pos, int y_pos)
{
    // Write on shared memory the new circle given the coordinates
    x_pos = 20 * x_pos;
    y_pos = 20 * y_pos;
...
// FUNCTIONS FOR DYNAMIC PRIVATE MEMORY
void remove_previous_circle(bmpfile_t *bmp)
{
    // Remove circle from private dynamic memory
    rgb_pixel_t *pixel;
...
{
    // Print circle on private dynamic memory
    for (int x = -radius; x <= radius; x++)
    {
        for (int y = -radius; y <= radius; y++)
        {
...
void read_input(int pos_x, int pos_y, rgb_pixel_t pixel, int *ptr, bmpfile_t *bmp)
{
...
int open_semaphores()
{
    // Open semaphores to write and read
